...
#### What is the purpose of having DOCTYPE?
...
* ```Readonly```: specifie that the input field is read only.
* ```Disabled```: is unusable and un-clickable and its value will not be sent when submittig the form.
...
* ```Placeholder```: specifies a hint that describes the expected value of an input filed.
